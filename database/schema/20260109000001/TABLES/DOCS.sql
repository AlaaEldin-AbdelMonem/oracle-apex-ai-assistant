...
--------------------------------------------------------
 ...
 JSON ("SECTION_MARKERS_JSON") STORE AS ( CHUNK 8192 RETENTION MIN 1800
  ...
  BUFFER_POOL DEFAULT FLASH_CACHE DEFAULT CELL_FLASH_CACHE DEFAULT)) ;
   ...
   COMMENT ON COLUMN "AI8P"."DOCS"."IS_ACTIVE" IS 'Active status flag: Y=Active, N=Inactive (soft delete)';
   COMMENT ON COLUMN "AI8P"."DOCS"."CHUNKING_STRATEGY" IS 'Chunking strategy to use for this document (NULL = auto-detect)';
   COMMENT ON COLUMN "AI8P"."DOCS"."CHUNK_SIZE_OVERRIDE" IS 'Override default chunk size for this document';
   COMMENT ON COLUMN "AI8P"."DOCS"."OVERLAP_PCT_OVERRIDE" IS 'Override default overlap percentage';
   COMMENT ON COLUMN "AI8P"."DOCS"."LAST_CHUNKED_AT" IS 'Timestamp of last chunking operation';
   COMMENT ON COLUMN "AI8P"."DOCS"."LAST_CHUNKING_STRATEGY" IS 'Strategy actually used in last chunking';
   COMMENT ON COLUMN "AI8P"."DOCS"."CHUNK_STATS_JSON" IS 'Cached chunk statistics: {"avg_size":1894,"min_size":512,"max_size":3231,"count":184}';
   COMMENT ON COLUMN "AI8P"."DOCS"."RECOMMENDED_STRATEGY" IS 'Cached chunking strategy recommendation from rag_chunk_strategy_pkg';
   COMMENT ON COLUMN "AI8P"."DOCS"."RECOMMENDATION_REASON" IS 'Explanation for why this strategy was recommended';
   COMMENT ON COLUMN "AI8P"."DOCS"."RECOMMENDATION_CONFIDENCE" IS 'Confidence score (0-100) for the recommendation';
   COMMENT ON COLUMN "AI8P"."DOCS"."RECOMMENDATION_UPDATED_AT" IS 'When the recommendation was last calculated';
   COMMENT ON COLUMN "AI8P"."DOCS"."TEXT_ORIGINAL" IS 'Original extracted text before enhancement';
   COMMENT ON COLUMN "AI8P"."DOCS"."ENHANCEMENT_APPLIED" IS 'Y/N flag indicating if text enhancement was applied';
   COMMENT ON COLUMN "AI8P"."DOCS"."ENHANCEMENT_STEPS" IS 'Comma-separated list of enhancement steps applied';
   COMMENT ON COLUMN "AI8P"."DOCS"."ENHANCEMENT_STATS" IS 'JSON object with before/after statistics';
   COMMENT ON COLUMN "AI8P"."DOCS"."LANGUAGE_DETECTED" IS 'ISO language code detected (en, ar, fr, etc.)';
   COMMENT ON COLUMN "AI8P"."DOCS"."TEXT_QUALITY_SCORE" IS 'Quality score 0-1 (1=highest quality)';
   COMMENT ON COLUMN "AI8P"."DOCS"."STRUCTURE_DETECTED" IS 'Document structure type (report, code, manual, etc.)';
   COMMENT ON COLUMN "AI8P"."DOCS"."DOC_TOC_JSON" IS 'Extracted table of contents as structured JSON';
   COMMENT ON COLUMN "AI8P"."DOCS"."DOC_INDEX_JSON" IS 'Extracted index as structured JSON with topics and page references';
   COMMENT ON COLUMN "AI8P"."DOCS"."DOC_INTRODUCTION" IS 'Introduction section extracted separately';
   COMMENT ON COLUMN "AI8P"."DOCS"."DOC_EXECUTIVE_SUMMARY" IS 'Executive summary (high-value content)';
   COMMENT ON COLUMN "AI8P"."DOCS"."DOC_PREFACE" IS 'Preface/foreword (typically removed from main content)';
   COMMENT ON COLUMN "AI8P"."DOCS"."HAS_STRUCTURED_SECTIONS" IS 'Y if document has identifiable sections (ToC, intro, index)';
   COMMENT ON COLUMN "AI8P"."DOCS"."SECTION_MARKERS_JSON" IS 'JSON array of section boundaries for intelligent chunking';
   COMMENT ON COLUMN "AI8P"."DOCS"."FRONT_MATTER_REMOVED" IS 'Y if copyright, ToC, and preface were removed from main text';
   COMMENT ON TABLE "AI8P"."DOCS"  IS 'Stores uploaded enterprise documents, metadata, extracted text, summaries, and AI processing info.';
